...
Saving.registerNumber("wishingWellLastAdClick", 0);
...
export class WishingWell extends Place {
    ...
    private drawCandiesStuff(x: number, y: number): void {
        ...
        // Else, we already threw our first candy
            ...
            if (localStorage.getItem("removeAds") !== "1") {
                ...
                this.renderArea.drawString("or", x, y + 2)

                const removeAdsStr = "Remove ads"
                this.renderArea.addAsciiRealButton(removeAdsStr, this.getRenderArea().getWidth() - removeAdsStr.length - 1, this.getRenderArea().getHeight() - 5, "wishingWellRemoveAds");
                this.renderArea.addLinkCall(".wishingWellRemoveAds", new CallbackCollection(() => {
                    // @ts-ignore
                    window.ReactNativeWebView?.postMessage("removeAds")
                }));

                const restorePurchasesStr = "Restore purchases"
                this.renderArea.addAsciiRealButton(restorePurchasesStr, this.getRenderArea().getWidth() - restorePurchasesStr.length - 1, this.getRenderArea().getHeight() - 2, "wishingWellRestorePurchases");
                this.renderArea.addLinkCall(".wishingWellRestorePurchases", new CallbackCollection(() => {
                    // @ts-ignore
                    window.ReactNativeWebView?.postMessage("restorePurchases")
                }));


                let lastAd = Saving.loadNumber("wishingWellLastAdClick")
                if (isNaN(lastAd)) lastAd = Date.now()

                let str = "Watch an ad"
                if (Date.now() < lastAd) {
                    str += ` (available in ${Math.floor((lastAd - Date.now()) / (1000))}s)`
                }

                this.renderArea.addAsciiRealButton(str, x, y + 4, "wishingWellWatchAd");
                this.renderArea.addLinkCall(".wishingWellWatchAd", new CallbackCollection(() => {
                    if (Date.now() < lastAd) return

                    Saving.saveNumber("wishingWellLastAdClick", Date.now() + 30 * 1000)
                    // @ts-ignore
                    window.ReactNativeWebView?.postMessage("adRequest")
                    this.update()
                    this.getGame().updatePlace();
                }));

                //TODO ad callback for ad
            } else {
                this.renderArea.addAsciiRealButton("Heal", x, y + 4, "wishingWellHeal");
                this.renderArea.addLinkCall(".wishingWellHeal", new CallbackCollection(() => {
    ...
    private update(): void {
        ...
        if (this.getGame().getLollipops().getMax() >= 1) this.drawLollipopsStuff(0, 9 + 4);
        if (this.getGame().getChocolateBars().getMax() >= 1) this.drawChocolateBarsStuff(0, 14 + 4);
        if (this.getGame().getPainsAuChocolat().getMax() >= 1) this.drawPainsAuChocolatStuff(0, 24 + 4);
    }
    ...
    public willBeDisplayed() {
        addEventListener("adReward", this.adReward)
    ...
    public willBeClosed() {
        removeEventListener("adReward", this.adReward)
    }
}