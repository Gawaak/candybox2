...
///<reference path="StringMethods.ts"/>
...
import {RenderLinkClick} from "./RenderLinkClick";
import {RenderLinkCallbackCollection} from "./RenderLinkCallbackCollection";
import {RenderLinkChange} from "./RenderLinkChange";
import {RenderLinkCheckbox} from "./RenderLinkCheckbox";
import {RenderLinkInput} from "./RenderLinkInput";
import {RenderLinkOnHoverShowTooltip} from "./RenderLinkOnHoverShowTooltip";
import {RenderLinkOver} from "./RenderLinkOver";
import {RenderLinkSimpleInput} from "./RenderLinkSimpleInput";
...
import {RenderLinkPointerDown} from "./RenderLinkPointerDown.ts";
import {RenderLinkPointerLeave} from "./RenderLinkPointerLeave.ts";
...
export class RenderArea {
    ...
    public addAsciiRealButton(str: string, x: number, y: number, otherClass: string = "", comment: string = "", commentBelow: boolean = false, underlinedLetter: number = -1, color: Color | null = null, commentCentered: boolean = true, commentRightAligned: boolean = false, fixed: boolean = false): boolean {
        // Add the button
        if (!this.addTag(new RenderTag(x, `<span class=\"aroundRealButton\" style='position: ${fixed ? 'fixed' : 'relative'}'><span class=\"asciiRealButton ` + otherClass + "\"" + (color == null ? "" : " style=\"background-color:" + color.getColorString() + ";\"") + ">" + Algo.makeUnderlinedLetter(str, underlinedLetter) + "</span></span>"), y))
            return false;
    ...
    public addCheckbox(x: number, y: number, callbackCollectionWhenChecked: CallbackCollection, callbackCollectionWhenUnchecked: CallbackCollection, otherClass: string, checkedByDefault: boolean = false): void {
    ...
    public addEnigma(x1: number, _x2: number, y: number, enigmaAnswer: EnigmaAnswer, callbackCollection: CallbackCollection, otherClass: string, wrongClass: string | null = null, wrongMessage: string = "Wrong"): void {
        ...
        enigmaAnswer.display(this, x1, y, otherClass, wrongClass)
    ...
    public addLinkLeaveCall(e: string, callbackCollection: CallbackCollection): void {
        this.links.push(new RenderLinkPointerLeave(e, callbackCollection));
    }

    public addLinkDownCall(e: string, callbackCollection: CallbackCollection): void {
        this.links.push(new RenderLinkPointerDown(e, callbackCollection));
    }
    ...
    public addLinkSimpleInput(e: string, callbackCollection: CallbackCollection, defaultValue: string | null, hasFocus: boolean): void {
    ...
    public addList(x1: number, x2: number, y: number, id: string, callbackCollection: CallbackCollection, optionsArray: string[] = []): void {
        ...
        this.addTag(new RenderTag(x1, "<span class=\"aroundSelect\"><select id=\"" + id + "\" class=\"asciiSelect\" style=\"width:" + ((x2 - x1) * this.getExValue()).toString() + "px\">" + options + "</select></span>"), y);
    ...
    public addSimpleInput(x1: number, x2: number, y: number, callbackCollection: CallbackCollection, otherClass: string, defaultValue: string | null = null, hasFocus: boolean = false): void {
        this.addTag(new RenderTag(x1, "<span class=\"aroundTextInput\"><input type=\"text\" class=\"asciiTextInput noHotkeys " + otherClass + "\" style=\"width:" + (x2 - x1).toString() + "ex\"></span>"), y);
    ...
    public addTextarea(x: number, y: number, _width: number, height: number, otherClass: string, text: string = ""): boolean {
        return this.addTag(new RenderTag(x, "<textarea class=\"textarea " + otherClass + "\" rows=\"" + height.toString() + "\">" + text + "</textarea>"), y);
    }
    ...
    public drawArea(renderArea: RenderArea, x: number = 0, y: number = 0, transparency: RenderTransparency | null = null): void {
    ...
    public drawArray(array: string[], x: number = 0, y: number = 0, transparency: RenderTransparency | null = null, spanClass: string | null = null) {
    ...
    public drawHorizontalLine(str: string, x1: number, x2: number, y: number): boolean {
        ...
        strBuffer = StringMethods.fillWith(strBuffer, str, x2 - x1)

        // We draw the string
        return this.drawString(strBuffer, x1, y);
    }

    public drawSpeech(str: string, y: number, x1: number, x2: number, speechClass: string | null = null, translatedText: string | null = null): number {
    ...
    public drawString(str: string, x: number = 0, y: number = 0, translated: boolean = false, transparency: RenderTransparency | null = null): boolean {
        ...
        if (transparency == null) {
            this.area[y] = StringMethods.replaceAt(this.area[y], x + indexFirst, str.substring(indexFirst, indexLast));
        }
            ...
            for (var i = indexFirst; i < indexLast; i++) { // We iterate over characters
                ...
                if (str[i] != transparency.getAlphaCharacter()) {
                    ...
                    if (transparency.getMetaAlphaCharacter() != null && str[i] == transparency.getMetaAlphaCharacter())
                        this.area[y] = StringMethods.replaceAt(this.area[y], x + i, transparency.getAlphaCharacter());
                    // Else, we draw the character
                    else
                        this.area[y] = StringMethods.replaceAt(this.area[y], x + i, str[i]);
                }
            }
        }
    ...
    public eraseEverything(character: string = " "): boolean {
        ...
        str = "";
        str = StringMethods.fillWith(str, character, this.width)
