...
import {Pos} from "./Pos";
import {CauldronAction} from "./CauldronAction";
import {CauldronActionLogEntry} from "./CauldronActionLogEntry";
import {CauldronFlame} from "./CauldronFlame";
// @ts-ignore
import $ from "jquery"

Saving.registerNumber("cauldronBookCurrentPage", 0);

export class Cauldron extends Place {
    // The render area
    ...
    private candiesInput: number = 0
    private lollipopsInput: number = 0;
    ...
    private candiesInputComment: string | null = null;
    ...
    // Action log
    private actionLog: (CauldronActionLogEntry | null)[] = [null, null, null, null, null]; // We always remember the last five actions

    // The comment telling the player the potions he managed to brew
    private potionsComment: string | null = null;
    ...
    private drawActions(x: number, y: number): void {
        // Variables
        ...
        [-500, -10, -1, 0, +1, +10, +500].map(i => {
            let str = ""
            if (i !== 0) {
                str = `   ${i > 0 ? '+' : ''}${i}`.slice(-4)
                this.renderArea.addAsciiRealButton(str, x + candiesX, y + 2, `cauldronAddCandy${i}`);
                this.renderArea.addLinkDownCall(`.cauldronAddCandy${i}`, new CallbackCollection(() => {
                    this.candiesInput = Math.max(0, this.candiesInput + i)
                    this.update();
                    this.getGame().updatePlace();
                }))
            } else {
                str = `candies: ${`       ${this.candiesInput.toString()}`.slice(-7)}`
                this.renderArea.drawString(str, x + candiesX, y + 2)
            }
            candiesX += str.length + 2
        });
        ...
        [-500, -10, -1, 0, +1, +10, +500].map(i => {
            ...
            if (i !== 0) {
                str = `   ${i > 0 ? '+' : ''}${i}`.slice(-4)
                this.renderArea.addAsciiRealButton(str, x + lollipopsX, y + 4, `cauldronAddLollipop${i}`);
                this.renderArea.addLinkDownCall(`.cauldronAddLollipop${i}`, new CallbackCollection(() => {
                    this.lollipopsInput = Math.max(0, this.lollipopsInput + i)
                    this.update();
                    this.getGame().updatePlace();
                }))
            } else {
                str = `lollipops: ${`    ${this.lollipopsInput.toString()}`.slice(-7)}`
                this.renderArea.drawString(str, x + lollipopsX, y + 4)
            }
            lollipopsX += str.length + 2
        })
    ...
    private putInCauldron(): void {
        ...
        const candies = this.candiesInput
        const lollipops = this.lollipopsInput
