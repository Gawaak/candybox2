...
import {Pos} from "./Pos";
import {LighthousePuzzlePart} from "./LighthousePuzzlePart";
import {LighthousePuzzlePartType} from "./LighthousePuzzlePartType";

export class LighthousePuzzle {
    // Array of parts
    ...
    private createParts(lighthouse: Lighthouse): void {
        ...
        for (var i = 0; i < 7; i++) {
            this.parts[i] = [];
            for (var j = 0; j < 5; j++) {
                this.parts[i][j] = new LighthousePuzzlePart(lighthouse, LighthousePuzzlePartType.BLANK, 1, new Pos());
            }
        }
