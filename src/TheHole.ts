...
export class TheHole extends Quest {
    ...
    constructor(game: Game) {
        ...
        // Resize the quest
        this.resizeQuest(this.getBaseWidth(), 35, new Pos(this.getBaseWidth(), 136));
    ...
    public willBeDisplayed(): void {
        ...
        this.getGame().addHotkey(new Hotkey("left", new CallbackCollection()));
        this.getGame().addHotkey(new Hotkey("right", new CallbackCollection()));
    ...
    public update(): void {
        ...
        if (this.getQuestEnded() == false) {
            ...
            this.getRenderArea().addAsciiRealButton("Move left", 5, 15)
            this.getRenderArea().addAsciiRealButton("Move right", this.getRenderArea().getWidth() - 5 - 9, 15)

            this.getRenderArea().addTwoTags(0, 20, 0, `<span class='moveLeftHole' style="height: 100%; display: block; position:absolute;">`, "</span>")
            this.getRenderArea().addTwoTags(this.getBaseWidth() - 20, this.getBaseWidth(), 0, `<span class='moveRightHole' style="height: 100%; display: block; position:absolute; right: 0">`, "</span>")

            this.getRenderArea().addLinkDownCall(".moveLeftHole", new CallbackCollection(() => {
               this.leftPressed = true
            }));
            this.getRenderArea().addLinkLeaveCall(".moveLeftHole", new CallbackCollection(() => {
                this.leftPressed = false
            }));

            this.getRenderArea().addLinkDownCall(".moveRightHole", new CallbackCollection(() => {
               this.rightPressed = true
            }));
            this.getRenderArea().addLinkLeaveCall(".moveRightHole", new CallbackCollection(() => {
    ...
    private moveHorizontally(): void {
        var leftPressed: boolean = Keyboard.isKeyPressed("left") || this.leftPressed
        var rightPressed: boolean = Keyboard.isKeyPressed("right") || this.rightPressed
