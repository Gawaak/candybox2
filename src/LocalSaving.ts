...
export module LocalSaving {
    ...
    export function load(slotId: string): boolean {
        ...
        for (var str in Saving.getAllBools()) {
            Saving.saveBool(str, LocalSaving.loadBool(slotId + "." + str));
        }

        // Load numbers
        for (var str in Saving.getAllNumbers()) {
            Saving.saveNumber(str, LocalSaving.loadNumber(slotId + "." + str));
        }

        // Load strings
        for (var str in Saving.getAllStrings()) {
            const loaded = LocalSaving.loadString(slotId + "." + str)
            if (loaded) Saving.saveString(str, loaded);
        }
