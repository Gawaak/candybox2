...
export class TheSea extends Quest {
    ...
    // Patterns stuff
    private currentPattern: TheSeaPattern | null = null;
    private currentPatternLevel: TheSeaPatternLevel | null = null;
    ...
    constructor(game: Game) {
        ...
        // Resize the quest
        this.resizeQuest(this.getBaseWidth(), 30);
    ...
    public willBeDisplayed(): void {
        ...
        this.getGame().addHotkey(new Hotkey("up", new CallbackCollection()));
        this.getGame().addHotkey(new Hotkey("down", new CallbackCollection()));
    }

    // Public methods
    public addBigShark(pos: Pos): BigShark | null {
    ...
    public addJellyFish(pos: Pos): JellyFish | null {
        var jellyFish: JellyFish = new JellyFish(this, pos);
        jellyFish.setHealthBar(new QuestEntityHealthBar(jellyFish, new Pos(6, 1), new Pos(0, 1)));
        if (this.addEntity(jellyFish)) // If we manage to add the entity
            return jellyFish; // We return it
        return null; // Else, we return null
    }

    public addMediumFish(pos: Pos): MediumFish | null {
        var mediumFish: MediumFish = new MediumFish(this, pos);
        mediumFish.setHealthBar(new QuestEntityHealthBar(mediumFish, new Pos(8, 1), new Pos(0, 1)));
        if (this.addEntity(mediumFish)) // If we manage to add the entity
            return mediumFish; // We return it
        return null; // Else, we return null
    }

    public addMiniShark(pos: Pos): MiniShark | null {
    ...
    public addSeahorse(pos: Pos, intendedXPosition: number): Seahorse | null {
    ...
    public addSeaSnake(pos: Pos): SeaSnake | null {
    ...
    public addSmallestFish(pos: Pos): SmallestFish | null {
    ...
    public castPlayerAcidRain(): void {
        ...
        super.castPlayerAcidRain(undefined, undefined, 0, 100);
    }
    ...
    public update(): void {
        ...
        if (this.getQuestEnded() == false) {
            this.getRenderArea().addAsciiRealButton("Move up", this.getRenderArea().getWidth() - 5 - 9, 5, "moveUpSea")
            this.getRenderArea().addAsciiRealButton("Move down", this.getRenderArea().getWidth() - 5 - 9, this.getRenderArea().getHeight() - 5, "moveDownSea")

            this.getRenderArea().addTwoTags(this.getBaseWidth() - 20, this.getBaseWidth(), 0, `<span class='moveUpSea' style="height: 50%; display: block; position:absolute; right: 0; top: 0;">`, "</span>")
            this.getRenderArea().addTwoTags(this.getBaseWidth() - 20, this.getBaseWidth(), 1, `<span class='moveDownSea' style="height: 50%; display: block; position:absolute; right: 0; bottom:0;">`, "</span>")

            this.getRenderArea().addLinkDownCall(".moveUpSea", new CallbackCollection(() => {
                this.upPressed = true
            }));
            this.getRenderArea().addLinkLeaveCall(".moveUpSea", new CallbackCollection(() => {
                this.upPressed = false
            }));

            this.getRenderArea().addLinkDownCall(".moveDownSea", new CallbackCollection(() => {
                this.downPressed = true
            }));
            this.getRenderArea().addLinkLeaveCall(".moveDownSea", new CallbackCollection(() => {
    ...
    // Private methods    
    private addFloor(index: number, type: TheSeaFloorType, height: number, previousFloor: TheSeaFloor | null = null): void {
        // Add the floor
    ...
    private moveVertically(): void {
        var upPressed: boolean = Keyboard.isKeyPressed("up") || this.upPressed;
        var downPressed: boolean = Keyboard.isKeyPressed("down") || this.downPressed;
