...
export module Saving {
    // Saving maps
    const bools: { [s: string]: boolean; } = {};
    const numbers: { [s: string]: number; } = {};
    const strings: { [s: string]: string; } = {};

    // Can we register?
    export const canRegister: boolean = true;
    ...
    export function load(game: Game, loadingType: MainLoadingType, loadingString: string): void {
        ...
        switch (loadingType) {
            ...
            case MainLoadingType.LOCAL:
                LocalSaving.load("autoSave");
                break;
            case MainLoadingType.FILE:
                var fileBools: string[] | null = loadingString.match(/bool +[a-zA-Z0-9_]+ *= *[a-zA-Z0-9_]+/g);
                var fileNumbers: string[] | null = loadingString.match(/number +[a-zA-Z0-9_]+ *= *[a-zA-Z0-9_]+/g);
                var fileStrings: string[] | null = loadingString.match(/string +[a-zA-Z0-9_]+ *= *[a-zA-Z0-9_]+/g);

                // Load the bools
                if (fileBools != null) {
                    for (var i = 0; i < fileBools.length; i++) {
                        Saving.saveBool(
                            fileBools[i].match(/[a-zA-Z0-9_]+=/)?.[0].replace("=", ""),
    ...
    export function stringToBool(s: string | undefined | null): boolean {
    ...
    export function stringToNumber(s: string | undefined | null): number {
        if (!s) {
            console.log("Error: trying to convert a string to number, but the string value is", s)
            return 0
        }
    ...
    export function loadNumber(key: string): number {
        ...
        if (key in numbers) return numbers[key];
    ...
    export function registerString(key: string, s: string): void {
        if (canRegister) {
            ...
            Saving.saveString(key, s, true);
        }
    }

    export function saveBool(key: string | undefined, b: boolean, registering: boolean = false): void {
        if (!key) return
    ...
    export function saveNumber(key: string | undefined, n: number, registering: boolean = false): void {
        if (!key) return
    ...
    export function saveString(key: string | undefined, s: string | undefined, registering: boolean = false): void {
        if (!key || !s) return
