...
import {QuestEntitySpellCaster} from "./QuestEntitySpellCaster";
...
export class QuestEntity {
    ...
    private healthBar: QuestEntityHealthBar | null = null;

    // Global position
    private globalPosition: Pos = new Pos(0, 0);

    // Ascii representation
    private renderAreaPosition: Pos = new Pos(0, 0);
    private renderArea: RenderArea | null = null;
    private transparency: RenderTransparency | null = null;

    // Collision box collection
    private cbc: CollisionBoxCollection | null = null;

    // Movement
    private questEntityMovement: QuestEntityMovement | null = null;
    private noMovementLastUpdate: boolean = true;

    // Animation
    private questEntityAnimation: QuestEntityAnimation | null = null;
    ...
    // Last damage reason
    private lastDamageReason: QuestEntityDamageReason | null = null;
    ...
    private spellCastingDamageReason: QuestEntityDamageReason | null = null;
    ...
    private antiGravityDuration: number = 0

    // Is a turtle?
    private turtle: boolean = false;
    private turtleDuration: number = 0
    private turtleLastMovement: number = 0

    // Is in berserk mode?
    private berserk: boolean = false;
    private berserkDuration: number = 0

    // Constructor
    init(quest: Quest | undefined,
         globalPosition: Pos,
         naming: Naming = new Naming("???", "???"),
         renderArea: RenderArea | null = null,
         renderAreaPosition: Pos = new Pos(0, 0),
         cbc: CollisionBoxCollection | null = null,
         questEntityMovement: QuestEntityMovement | null = null,
         questEntityAnimation: QuestEntityAnimation | null = null
    ) {
        // Apply parameters
        this.quest = quest;
    ...
    public draw(renderArea: RenderArea): void {
        if (this.renderArea != null && !!this.quest) {
        ...
        if (Saving.loadBool("gameDebug") && !!this.quest) {
            if (this.cbc != null) {
    ...
    public inflictDamage(damage: number, reason: QuestEntityDamageReason): void {
        ...
        if (this.destructible) {
            ...
            if (this.turtle == false) {
                ...
                const hp = this.getHp()
                if (hp !== undefined) this.setHp(hp - damage);
            } else {
                // Else, we're a turtle
                const hp = this.getHp()
                if (hp !== undefined) this.setHp(hp - Math.ceil(damage / 2));
            }
        }
    }
    ...
    public isOutOfArea(): boolean {
        ...
        if (this.globalPosition.y < -this.getQuest().getTopLimit())
            return true;
        if (!!this.quest && this.globalPosition.x > this.quest.getRealQuestSize().x + this.getQuest().getRightLimit())
            return true;
        if (!!this.quest && this.globalPosition.y > this.quest.getRealQuestSize().y + this.getQuest().getBottomLimit())
            return true;
    ...
    public shouldDie(): boolean {
        ...
        if ((this.destructible && (!this.hp || this.hp <= 0)) || this.dead)
            return true;
    ...
    public getCbc(): CollisionBoxCollection {
        return this.cbc ?? new CollisionBoxCollection();
    }
    ...
    public getHealthBar(): QuestEntityHealthBar | null {
    ...
    public getLastDamageReason(): QuestEntityDamageReason | null {
    ...
    public getQuestEntityAnimation(): QuestEntityAnimation {
        return this.questEntityAnimation || new QuestEntityAnimation(0, 0, 0);
    }

    public getQuestEntityMovement(): QuestEntityMovement {
        return this.questEntityMovement || new QuestEntityMovement();
    }
    ...
    public getRenderArea(): RenderArea {
        return this.renderArea ?? new RenderArea();
    }
    ...
    public setCbc(cbc: CollisionBoxCollection | null): void {
        this.cbc = cbc;
    }
    ...
    public setHp(hp: number): void {
        ...
        if (!!this.hp && this.hp < 0) this.hp = 0;
    ...
    public setQuestEntityAnimation(questEntityAnimation: QuestEntityAnimation | null): void {
    ...
    public setQuestEntityMovement(questEntityMovement: QuestEntityMovement | null): void {
        this.questEntityMovement = questEntityMovement;
    }

    public setRenderArea(renderArea: RenderArea | null): void {
    ...
    public setTransparency(transparency: RenderTransparency | null): void {
