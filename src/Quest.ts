...
import {QuestPlayerSpell} from "./QuestPlayerSpell";
import {QuestPlayerSpellCountdownType} from "./QuestPlayerSpellCountdownType";
import {PlayerSummonedDemon} from "./PlayerSummonedDemon";
import {PlayerCharacterType} from "./PlayerCharacterType";
import {PlayerCloneCandyBox} from "./PlayerCloneCandyBox";
import {PlayerCloneMedium} from "./PlayerCloneMedium";
...
Saving.registerNumber("lastSpell1", 0);
Saving.registerNumber("lastSpell2", 0);
Saving.registerNumber("lastSpell3", 0);
...
export class Quest extends Place {
    ...
    private spellsBarLocation: RenderLocation = new RenderLocation("#spellsBar");
    private spellsButtonsLocation: RenderLocation = new RenderLocation("#spellsButtons");
    private questLogLocation: RenderLocation = new RenderLocation("#questLog");

    // Render area
    private renderArea: RenderArea = new RenderArea();
    private spellBarRenderArea: RenderArea = new RenderArea(100, 1);
    private spellBarButtonsRenderArea: RenderArea = new RenderArea(100, 1);
    private questLogArea: RenderArea = new RenderArea(100, 6)

    private realQuestPosition: Pos; // Indicates where the daughter class should draw its stuff
    private realQuestSize: Pos = new Pos(0, 0); // The real quest size
    private realQuestDrawingSize: Pos = new Pos(0, 0); // Only different from realQuestSize if realQuestSize is forced when calling the resize() method
    ...
    private lastSpells: number[] = []
    private shownSpellsCategory: QuestPlayerSpellCountdownType | null = null
    ...
    // Special instruction
    private specialInstruction: string | null;
    ...
    // Constructor
    constructor(game: Game, specialInstruction: string | null = null) {
        super(game);
        ...
        this.sizeToAddToTheRealQuestSize = new Pos(0, 0);
        ...
        if (this.playerSpells.length != 0) {
            this.realQuestPosition.add(new Pos(0, 0));
        }
    ...
    public addExitQuestButton(callbackCollection: CallbackCollection, buttonText: string): void {
        this.addBackToButton(this.renderArea,
            ...
            buttonText,
            0,
            ((this.renderArea.getWidth() - this.getBaseWidth()) - this.getGap()) / 2 + (50 - Database.getText(buttonText).length / 2)
        );
    }

    public castPlayerAcidRain(areaPosition: Pos | null = null, areaSize: Pos | null = null, maxLeftDistanceFromPlayer: number = 50, maxRightDistanceFromPlayer: number = 50): void {
    ...
    public castPlayerCloningPotion(): void {
        ...
        var clone: QuestEntity | null = null;
    ...
    public castPlayerPPotion(): void {
        ...
        var additionalMessage: string | null = null;
    ...
    public castPlayerTeleport(where: Pos | null = null, teleportArea: Pos | null = null): void {
    ...
    public configPlayerOrClone(_entity: QuestEntity): void {
    ...
    public drawEntities(): void {
        for (var i = 0; i < this.entities.length; i++) {
            ...
            const entity = this.entities[i].getHealthBar()
            if (entity != null) entity.draw(this.renderArea);
        }
    }
    ...
    public preDraw(): void {
        ...
        this.spellBarRenderArea.resetAllButSize();
        this.spellBarButtonsRenderArea.resetAllButSize()
        this.questLogArea.resetAllButSize()
    }

    public resizeQuest(width: number, height: number, forcedRealQuestSize: Pos | null = null): void {
    ...
    public getGap(): number {
        const size = this.getBaseWidth()
        let gap: number = (this.renderArea.getWidth() - size) - (this.getGame().getPlayer().getGlobalPosition().x - size * 0.5) * 2;

        // We possibly correct this gap to keep the quest in the center if the player is in the left or right of the quest
        if (gap > (this.renderArea.getWidth() - size)) gap = (this.renderArea.getWidth() - size);
        if (gap < -(this.renderArea.getWidth() - size)) gap = -(this.renderArea.getWidth() - size);
    ...
    public getBaseWidth(): number {
        if (!this.baseWidth) {
            this.baseWidth = $("#contentContainer").width() / $("#getExValue").width()
        }
    ...
    private addToLastSpells(spellIndex: number) {
        if (this.lastSpells.includes(spellIndex)) {
            return
        }
        this.lastSpells.push(spellIndex)
        while (this.lastSpells.length > 3) {
            this.lastSpells.shift()
        }
        for (let i = 0; i < 3; i++) {
            Saving.saveNumber(`lastSpell${i + 1}`, this.lastSpells[i])
        }
    }

    // Private methods
    private addPlayerSpell(playerSpell: QuestPlayerSpell): void {
        const index = this.playerSpells.length
        this.playerSpells.push(playerSpell);
        playerSpell.addCallbackOnCast(() => {
            this.shownSpellsCategory = null
            this.hideSpellsButtons()
            this.addToLastSpells(index)
        })
    ...
    private createPlayerSpells(): void {
        ...
        if (Saving.loadBool("gridItemPossessedBeginnersGrimoire")) {
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellAcidRainButton", new Pos(1, 0), "Acid rain", new Color(ColorType.QUEST_BUTTON_ACID_RAIN), new CallbackCollection(this.castPlayerAcidRain.bind(this)), QuestPlayerSpellCountdownType.SPELLS, this.getMagicCountdownTime(), 0, "a"));
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellFireballButton", new Pos(12, 0), "Fireball", new Color(ColorType.QUEST_BUTTON_FIREBALL), new CallbackCollection(this.castPlayerFireball.bind(this)), QuestPlayerSpellCountdownType.SPELLS, this.getMagicCountdownTime(), 0, "f"));
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellTeleportButton", new Pos(23, 0), "Teleport", new Color(ColorType.QUEST_BUTTON_TELEPORT), new CallbackCollection(this.castPlayerTeleport.bind(this)), QuestPlayerSpellCountdownType.SPELLS, this.getMagicCountdownTime(), 0, "t"));
        }
        if (Saving.loadBool("gridItemPossessedAdvancedGrimoire")) {
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellEraseMagicButton", new Pos(34, 0), "Erase magic", new Color(ColorType.QUEST_BUTTON_ERASE_MAGIC), new CallbackCollection(this.castPlayerEraseMagic.bind(this)), QuestPlayerSpellCountdownType.SPELLS, this.getMagicCountdownTime(), 0, "e"));
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellThornsShieldButton", new Pos(47, 0), "Thorns shield", new Color(ColorType.QUEST_BUTTON_THORNS_SHIELD), new CallbackCollection(this.castPlayerThornsShield.bind(this)), QuestPlayerSpellCountdownType.SPELLS, this.getMagicCountdownTime(), 6, " "));
        }
        if (Saving.loadBool("gridItemPossessedBlackMagicGrimoire")) {
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellObsidianWallButton", new Pos(64, 0), "Obsidian wall", new Color(ColorType.QUEST_BUTTON_OBSIDIAN_WALL), new CallbackCollection(this.castPlayerObsidianWall.bind(this)), QuestPlayerSpellCountdownType.SPELLS, this.getMagicCountdownTime(), 0, "o"));
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellBlackDemonsButton", new Pos(79, 0), "Black demons", new Color(ColorType.QUEST_BUTTON_BLACK_DEMONS), new CallbackCollection(this.castPlayerBlackDemons.bind(this)), QuestPlayerSpellCountdownType.SPELLS, this.getMagicCountdownTime(), 6, "d"));
        }

        // Add the potions
        if (Saving.loadBool("questPlayerSpellHealthPotionHasSpell"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellHealthPotionButton", new Pos(1, 0), "Health", new Color(ColorType.QUEST_BUTTON_HEALTH_POTION), new CallbackCollection(this.castPlayerHealthPotion.bind(this)), QuestPlayerSpellCountdownType.POTIONS, this.getMagicCountdownTime(), 0, "h", "questPlayerSpellHealthPotionQuantity"));
        if (Saving.loadBool("questPlayerSpellTurtlePotionHasSpell"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellTurtlePotionButton", new Pos(14, 0), "Turtle", new Color(ColorType.QUEST_BUTTON_TURTLE_POTION), new CallbackCollection(this.castPlayerTurtlePotion.bind(this)), QuestPlayerSpellCountdownType.POTIONS, this.getMagicCountdownTime(), 2, "r", "questPlayerSpellTurtlePotionQuantity"));
        if (Saving.loadBool("questPlayerSpellAntiGravityPotionHasSpell"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellAntiGravityPotionButton", new Pos(27, 0), "Anti gravity", new Color(ColorType.QUEST_BUTTON_ANTI_GRAVITY_POTION), new CallbackCollection(this.castPlayerAntiGravityPotion.bind(this)), QuestPlayerSpellCountdownType.POTIONS, this.getMagicCountdownTime(), 5, "g", "questPlayerSpellAntiGravityPotionQuantity"));
        if (Saving.loadBool("questPlayerSpellBerserkPotionHasSpell"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellBerserkPotionButton", new Pos(46, 0), "Berserk", new Color(ColorType.QUEST_BUTTON_BERSERK_POTION), new CallbackCollection(this.castPlayerBerserkPotion.bind(this)), QuestPlayerSpellCountdownType.POTIONS, this.getMagicCountdownTime(), 0, "b", "questPlayerSpellBerserkPotionQuantity"));
        if (Saving.loadBool("questPlayerSpellCloningPotionHasSpell"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellCloningPotionButton", new Pos(60, 0), "Cloning", new Color(ColorType.QUEST_BUTTON_CLONING_POTION), new CallbackCollection(this.castPlayerCloningPotion.bind(this)), QuestPlayerSpellCountdownType.POTIONS, this.getMagicCountdownTime(), 0, "c", "questPlayerSpellCloningPotionQuantity"));
        if (Saving.loadBool("questPlayerSpellPPotionHasSpell"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellPPotionButton", new Pos(74, 0), "P", new Color(ColorType.QUEST_BUTTON_P_POTION), new CallbackCollection(this.castPlayerPPotion.bind(this)), QuestPlayerSpellCountdownType.POTIONS, this.getMagicCountdownTime(), 0, "p", "questPlayerSpellPPotionQuantity"));
        if (Saving.loadBool("questPlayerSpellXPotionHasSpell"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellXPotionButton", new Pos(82, 0), "X", new Color(ColorType.QUEST_BUTTON_X_POTION), new CallbackCollection(this.castPlayerXPotion.bind(this)), QuestPlayerSpellCountdownType.POTIONS, this.getMagicCountdownTime(), 0, "x", "questPlayerSpellXPotionQuantity"));

        // Add the capacities granted by objects
        if (Saving.loadBool("gridItemPossessedTimeRing"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellTimeSlowingButton", new Pos(1, 0), "Time slowing", new Color(ColorType.QUEST_BUTTON_SOME_OBJECT), new CallbackCollection(this.castPlayerTimeSlowing.bind(this)), QuestPlayerSpellCountdownType.ITEM_CAPACITIES, 0, 8, "w"));
        if (Saving.loadBool("gridItemPossessedPogoStick"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellJumpButton", new Pos(15, 0), "Jump", new Color(ColorType.QUEST_BUTTON_SOME_OBJECT), new CallbackCollection(this.castPlayerJump.bind(this)), QuestPlayerSpellCountdownType.ITEM_CAPACITIES, 0, 0, "j"));
        if (Saving.loadBool("gridItemPossessedSponge"))
            this.addPlayerSpell(new QuestPlayerSpell(this, "questPlayerSpellSqueezeButton", new Pos(21, 0), "Squeeze", new Color(ColorType.QUEST_BUTTON_SOME_OBJECT), new CallbackCollection(this.castPlayerSqueeze.bind(this)), QuestPlayerSpellCountdownType.ITEM_CAPACITIES, 0, 0, "s"));
        ...
        for (let i = 0; i < 3; i++) {
            const spell = Saving.loadNumber(`lastSpell${i + 1}`)
            if (!spell) break

            this.lastSpells.push(spell)
        }
    ...
    private toggleSpellsDisplay(key: QuestPlayerSpellCountdownType): void {
        if (this.shownSpellsCategory === key) {
            this.shownSpellsCategory = null
            this.hideSpellsButtons()
        } else {
            this.shownSpellsCategory = key
            this.showSpellsButtons()
        }
    }

    private hideSpellsButtons() {
        $("#aroundSpellsButtons").css("display", "none")
    }

    private showSpellsButtons() {
        $("#aroundSpellsButtons").css("display", "flex")
    }

    private drawPlayerSpellsArea(): void {
        for (const spell of this.playerSpells) {
            if (spell.getCountdownType() !== this.shownSpellsCategory) continue

            spell.draw(this.spellBarButtonsRenderArea, new Pos(0, 0), "relativePos");
        }
        this.spellsButtonsLocation.render(this.spellBarButtonsRenderArea)
    }

    private drawPlayerSpells(): void {
        if (Saving.loadBool("gridItemPossessedPurpleSharkFin") && this.canCastBlackHole) {
            this.spellBarRenderArea.addAsciiRealButton("BLACK HOLE", 0, 0, "castBlackHole relativePos");
            this.renderArea.addLinkDownCall(".castBlackHole", new CallbackCollection(() => {
                this.canCastBlackHole = false
                this.castPlayerBlackhole()
            }));
        }

        if (Saving.loadBool("gridItemPossessedTimeRing")
            || Saving.loadBool("gridItemPossessedPogoStick")
            || Saving.loadBool("gridItemPossessedSponge")) {

            this.spellBarRenderArea.addAsciiRealButton("ITEMS", 0, 0, "toggleItemsButton relativePos");
            this.renderArea.addLinkDownCall(".toggleItemsButton", new CallbackCollection(() => this.toggleSpellsDisplay(QuestPlayerSpellCountdownType.ITEM_CAPACITIES)));
        }

        if (Saving.loadBool("questPlayerSpellHealthPotionHasSpell")
            || Saving.loadBool("questPlayerSpellTurtlePotionHasSpell")
            || Saving.loadBool("questPlayerSpellAntiGravityPotionHasSpell")
            || Saving.loadBool("questPlayerSpellBerserkPotionHasSpell")
            || Saving.loadBool("questPlayerSpellCloningPotionHasSpell")
            || Saving.loadBool("questPlayerSpellPPotionHasSpell")
            || Saving.loadBool("questPlayerSpellXPotionHasSpell")) {

            let str = "POTIONS"
            if (this.getPlayerPotionsCountdown() > 0) {
                str = `POTS(${Math.floor(this.getPlayerPotionsCountdown() / 10)})`
            }

            this.spellBarRenderArea.addAsciiRealButton(str, 0, 0, "togglePotionsButton relativePos");
            this.renderArea.addLinkDownCall(".togglePotionsButton", new CallbackCollection(() => this.toggleSpellsDisplay(QuestPlayerSpellCountdownType.POTIONS)));
        }

        if (Saving.loadBool("gridItemPossessedBeginnersGrimoire")) {

            let str = "SPELLS"
            if (this.getPlayerSpellsCountdown() > 0) {
                str = `SPE(${Math.floor(this.getPlayerSpellsCountdown() / 10)})`
            }

            this.spellBarRenderArea.addAsciiRealButton(str, 0, 0, "toggleSpellsButton relativePos");
            this.renderArea.addLinkDownCall(".toggleSpellsButton", new CallbackCollection(() => this.toggleSpellsDisplay(QuestPlayerSpellCountdownType.SPELLS)));
        }


        for (let i = 0; i < this.lastSpells.length; i++) {
            const spell = this.playerSpells[this.lastSpells[i]]
            spell.draw(this.spellBarRenderArea, new Pos(), "relativePos")
        }

        this.spellsBarLocation.render(this.spellBarRenderArea)
    }

    private drawQuestLog(): void {
        this.getGame().getQuestLog().draw(this.questLogArea, new Pos(0, 0));
        this.questLogArea.addAsciiRealButton(this.questLogOpened ? " \\/ " : " /\\ ", this.questLogArea.getWidth() - 3, 0, "toggleQuestLog")
        this.renderArea.addLinkDownCall(".toggleQuestLog", new CallbackCollection(() => {
            this.questLogOpened = !this.questLogOpened
            $("#questLogContainer").css("height", this.questLogOpened ? "18svh" : "3svh")
        }))

        this.questLogLocation.render(this.questLogArea)
    }
