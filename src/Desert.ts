...
import {DesertBird} from "./DesertBird";
...
export class Desert extends Quest{
    ...
    private addBird(): void{
        ...
        for(var i = 0; i < this.getEntities().length; i++){
            if(this.getEntities()[i].getCbc() != null && this.getEntities()[i].getCbc().collidesWith(new CollisionBoxCollection(new CollisionBox(new QuestEntity().init(this, new Pos(0, 0)), new Pos(0, yPos), new Pos(149, 4)))))
                return;
        }
