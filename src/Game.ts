...
import {ChocolateBars} from "./ChocolateBars";
import {PainsAuChocolat} from "./PainsAuChocolat";
import {CandiesEaten} from "./CandiesEaten";
import {CandiesThrown} from "./CandiesThrown";
...
import {CastleEntrance} from "./CastleEntrance";
...
import {MonkeyWizardStaff} from "./MonkeyWizardStaff";
import {EnchantedMonkeyWizardStaff} from "./EnchantedMonkeyWizardStaff";
import {TribalSpear} from "./TribalSpear";
import {SummoningTribalSpear} from "./SummoningTribalSpear";
import {Scythe} from "./Scythe";
import {OctopusKingCrown} from "./OctopusKingCrown";
import {OctopusKingCrownWithJaspers} from "./OctopusKingCrownWithJaspers";
import {SorceressHat} from "./SorceressHat";
import {EnchantedKnightBodyArmour} from "./EnchantedKnightBodyArmour";
import {OctopusKingCrownWithObsidian} from "./OctopusKingCrownWithObsidian";
import {RedEnchantedGloves} from "./RedEnchantedGloves";
import {PinkEnchantedGloves} from "./PinkEnchantedGloves";
import {BootsOfIntrospection} from "./BootsOfIntrospection";
import {Pos} from "./Pos";
...
import {LeatherGloves} from "./LeatherGloves";
import {KnightBodyArmour} from "./KnightBodyArmour";
import {LightweightBodyArmour} from "./LightweightBodyArmour";
import {MerchantHat} from "./MerchantHat";
import {GiantSpoonOfDoom} from "./GiantSpoonOfDoom";
import {GiantSpoon} from "./GiantSpoon";
import {IronAxe} from "./IronAxe";
import {MainMap} from "./MainMap";
import {LollipopFarm} from "./LollipopFarm";
import {Lighthouse} from "./Lighthouse";
import {Inventory} from "./Inventory";
import {InsideYourBox} from "./InsideYourBox";
import {InsideFortress} from "./InsideFortress";
...
export class Game {
    ...
    private savedPlace: Place | null = null;
    ...
    private questSpeedUp: number = 0;

    // Local autosave
    private localAutosaveEnabled: boolean = false;
    private localAutosaveSlot: string | null = null;
    private localAutosaveTime: number | null = null; // Time in seconds before the next save
    ...
    constructor(gameMode: string) {
        ...
        const fontSize: number = parseInt(statusBar.css("fontSize"))
        const height: number = parseInt(statusBar.css("height"))

        statusBar.css("line-height", height / fontSize / 29)
        ...
        setTimeout(() => {
            ...
            window.saveInterval = setInterval(() => {
                ...
                Saving.save(this, MainLoadingType.LOCAL, "autoSave");
            }, 1000)
        }, 1000)
    }
    ...
    public gainItem(itemSavingName: string): void {
    ...
    public setPlace(place: Place): void {
        ...
        this.place.hideSpellBar()
        this.place.hideQuestLog()
    ...
    public unequipIfEquipped(savingName: string, type: EqItemType): void {
        switch (type) {
            case EqItemType.WEAPON:
                if (this.selectedEqItems["weapon"] != null && this.selectedEqItems["weapon"].getSavingName() == savingName) Saving.saveString("gameWeaponSelected", "inventorySpecialNothingWeapon");
                break;
            case EqItemType.HAT:
                if (this.selectedEqItems["hat"] != null && this.selectedEqItems["hat"].getSavingName() == savingName) Saving.saveString("gameHatSelected", "inventorySpecialNothingHat");
                break;
            case EqItemType.BODYARMOUR:
                if (this.selectedEqItems["bodyArmour"] != null && this.selectedEqItems["bodyArmour"].getSavingName() == savingName) Saving.saveString("gameBodyArmourSelected", "inventorySpecialNothingBodyArmour");
                break;
            case EqItemType.GLOVES:
                if (this.selectedEqItems["gloves"] != null && this.selectedEqItems["gloves"].getSavingName() == savingName) Saving.saveString("gameGlovesSelected", "inventorySpecialNothingGloves");
                break;
            case EqItemType.BOOTS:
                if (this.selectedEqItems["boots"] != null && this.selectedEqItems["boots"].getSavingName() == savingName) Saving.saveString("gameBootsSelected", "inventorySpecialNothingBoots");
                break;
        }
    ...
    public getLocalAutosaveTime(): number {
        return <number>this.localAutosaveTime;
    }
    ...
    private displayArea(renderArea: RenderArea, scrolling: boolean = false, gap: number = 0, defaultScroll: Pos = new Pos()): void {
    ...
    private displayPlace(): void {
        if (!this.place) {
            console.log("Error: Place is null")
            return
        }
