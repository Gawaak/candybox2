...
Saving.registerNumber("mainMapDefaultScrollX", 400);
Saving.registerNumber("mainMapDefaultScrollY", 400);
...
export class MainMap extends Place {
    ...
    public willStopBeingDisplayed(): void {
        const scroll = this.getGame().getMainContentLocation().getScroll()
        Saving.saveNumber("mainMapDefaultScrollX", scroll.x);
        Saving.saveNumber("mainMapDefaultScrollY", scroll.y);
    }

    // Public getters
    public getDefaultScroll(): Pos {
        return new Pos(Saving.loadNumber("mainMapDefaultScrollX"), Saving.loadNumber("mainMapDefaultScrollY"))
    }
