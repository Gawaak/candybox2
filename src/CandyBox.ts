...
export class CandyBox extends Place {
    ...
    public update(): void {
        ...
        const get1Candy = "Get one candy"
        this.renderArea.addAsciiRealButton(get1Candy, this.renderArea.getWidth() - get1Candy.length - 1, 1, "candyBoxGetOneCandyButton");
        this.renderArea.addLinkCall(".candyBoxGetOneCandyButton", new CallbackCollection(() => {
    ...
    private requestStatusBarUnlockedHealthBar(): void {
        if (this.getGame().getCandies().getCurrent() >= 5) {
            ...
            this.getGame().updateStatusBar(true);
            this.getGame().getPlayer().heal(100)
            this.update();
            this.getGame().updatePlace();
        }
    }
